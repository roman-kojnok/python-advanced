...
# Here we will add functionality to add the information dictionary. The new key is defined in the URL, and the value
# of the key is in the sent data. We also want to add a check for pre-existence, so that we do not update existing
# entires (we want to save that for PUT requests).
...
# We will implement update functionality (PUT request) with the same URL as the route for POST requests, but with a
# PUT method. Similar to before, we want to check the dictionary for pre-existence so that we only implement changes
